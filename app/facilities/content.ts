export const FACILITIES = [
  {
    name: "Fireplace",
    description:
      "The perfect place to gather and relax on cold winter evenings.",
    imagePaths: [
      "/sub_pages/facilities/fireplace/fireplace_1.png",
      "/sub_pages/facilities/fireplace/fireplace_2.png",
    ],
  },
  {
    name: "Communal Kitchen",
    description:
      "Fully equipped with two stoves, two ovens, two microwaves and a multitude of kitchenware.",
    imagePaths: [
      "/sub_pages/facilities/communal_kitchen/communal_kitchen_1.png",
      "/sub_pages/facilities/communal_kitchen/communal_kitchen_2.png",
    ],
  },
  {
    name: "Shared Bathroom",
    description:
      "One on each level per gender, with newly refurbished showers.",
    imagePaths: [
      "/sub_pages/facilities/shared_bathroom/shared_bathroom_1.png",
      "/sub_pages/facilities/shared_bathroom/shared_bathroom_2.png",
    ],
  },
  {
    name: "Laundry Room",
    description:
      "Outfitted with 2 washers and 2 dryers for your cleaning needs.",
    imagePaths: [
      "/sub_pages/facilities/laundry_room/laundry_room_1.png",
      "/sub_pages/facilities/laundry_room/laundry_room_2.png",
    ],
  },
  {
    name: "TV Lounge",
    description: 'Full 65" TV with Netflix, Amazon Prime and plenty of DVDs.',
    imagePaths: [
      "/sub_pages/facilities/tv_lounge/tv_lounge_1.png",
      "/sub_pages/facilities/tv_lounge/tv_lounge_2.png",
    ],
  },
  {
    name: "Balcony",
    description:
      "Great for catching a picturesque view of the sunset or stargazing on clear nights.",
    imagePaths: [
      "/sub_pages/facilities/balcony/balcony_1.png",
      "/sub_pages/facilities/balcony/balcony_2.png",
    ],
  },
  {
    name: "Courtyard",
    description: "BBQ pits available for grilling.",
    imagePaths: [
      "/sub_pages/facilities/courtyard/courtyard_1.png",
      "/sub_pages/facilities/courtyard/courtyard_2.png",
    ],
  },
  {
    name: "Car Park",
    description: "Free, accessible and secure parking on premise.",
    imagePaths: [
      "/sub_pages/facilities/car_park/car_park_1.png",
      "/sub_pages/facilities/car_park/car_park_2.png",
    ],
  },
  {
    name: "Amuri Avenue Reserve",
    description:
      "Enjoy a 5 minutes walk down a stretch of oak trees on your way to the hot pools. ",
    imagePaths: [
      "/sub_pages/facilities/amuri_avenue_reserve/amuri_avenue_reserve_1.png",
      "/sub_pages/facilities/amuri_avenue_reserve/amuri_avenue_reserve_2.png",
    ],
  },
];

export const SERVICES = [
  { iconName: "fa-solid fa-wifi", description: "Free wifi" },
  { iconName: "fa-solid fa-suitcase-rolling", description: "Luggage storage" },
  { iconName: "fa-solid fa-box", description: "Parcel drop-off" },
  { iconName: "fa-solid fa-book", description: "Book exchange" },
];

export const PURCHASEABLES = [
  {
    iconName: "fa-solid fa-ticket",
    description: "Pool tickets @ $37 (regular price: $40)",
  },
  { iconName: "fa-solid fa-rug", description: "Towel hire @ $2" },
  { iconName: "fa-solid fa-bicycle", description: "Bike hire @ $20" },
  { iconName: "fa-solid fa-jug-detergent", description: "Laundry powder @ $1" },
  { iconName: "fa-solid fa-bowl-food", description: " Food kiosk" },
];
